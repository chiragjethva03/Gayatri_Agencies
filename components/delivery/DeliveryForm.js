...
export default function DeliveryForm({ isOpen, onClose, onSaveSuccess, initialData, isViewMode = false }) {
  ...
  const fetchLocations = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const currentTransport = data.find(t => t.name.toLowerCase().replace(/\s+/g, '-') === slug);
        if (currentTransport && currentTransport.locations) {
  ...
  useEffect(() => {
    ...
    setFormData(prev => {
      if (prev.totalFreight !== formData.amount || prev.deliverySubTotal !== subTotal || prev.deliveryFreight !== finalFreight) {
        return {
          ...prev, totalFreight: formData.amount, deliverySubTotal: subTotal || "", deliveryFreight: finalFreight || "" 
        };
      }
      return prev;
    });
  ...
  const handleChange = (e) => {
    ...
    if (name === "lrNoInput" || name === "article") setFormData((prev) => ({ ...prev, [name]: value.replace(/\D/g, "") })); 
    else if (["weight", "rate", "amount", "hamali", "serviceCharge", "discount"].includes(name)) setFormData((prev) => ({ ...prev, [name]: value.replace(/[^0-9.]/g, "") }));
  ...
  const handleGetLr = async () => {
    ...
    if (lrList.some(lr => lr.lrNo === lrInput)) return alert(`LR No ${lrInput} is already in the table.`);

    try {
      const res = await fetch(`/api/lr?transport=${slug}`);
      if (!res.ok) throw new Error("Failed to fetch");
      const data = await res.json();
      const foundLr = data.find(lr => lr.lrNo === lrInput);
      ...
      const newRow = {
        id: foundLr._id, lrNo: foundLr.lrNo, lrDate: foundLr.lrDate || "-", from: foundLr.fromCity || "-", to: foundLr.toCity || "-",
        consignor: foundLr.consignor || "-", consignorGst: "-", consignee: foundLr.consignee || "-", consigneeGst: "-", 
        pack: packNames, description: desc, freightBy: foundLr.freightBy || "-", article: totalArt, weight: totalWt, amount: freightAmount
      };

      setLrList(prev => [...prev, newRow]);
      setFormData(prev => ({ ...prev, lrNoInput: "" })); 
    } catch (err) { alert("Error fetching LR details."); }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-2">
      <div className="bg-white w-full max-w-[95vw] h-[95vh] flex flex-col shadow-xl border border-gray-400 text-xs rounded-sm overflow-hidden">
        ...
        <div className="bg-[#1e73be] text-white px-3 py-1.5 flex justify-between items-center shrink-0">
          {/* UPDATED HEADER TITLE FOR VIEW MODE */}
          ...
          <div className="p-3 bg-[#f8fafc] border-b border-gray-300 shrink-0">
            <div className="flex items-end gap-3 mb-3">
              <div className="flex flex-col w-48">
                ...
                <input disabled={isViewMode} type="text" name="lrNoInput" value={formData.lrNoInput} onChange={handleChange} onKeyDown={(e) => e.key === "Enter" && handleGetLr()} className={`border border-blue-300 rounded p-1.5 text-xs focus:outline-blue-500 ${isViewMode ? 'bg-gray-100 cursor-not-allowed' : 'bg-[#eef2ff]'}`} />
              </div>
              <button disabled={isViewMode} className={`px-4 py-1.5 rounded text-xs font-semibold shadow-sm text-white ${isViewMode ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#1e73be] hover:bg-blue-700'}`}>Go</button>
              <button disabled={isViewMode} onClick={handleGetLr} type="button" className={`px-4 py-1.5 rounded text-xs font-semibold shadow-sm text-white ${isViewMode ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#1e73be] hover:bg-blue-700'}`}>Get Lr</button>
            ...
            <div className="flex gap-4">
              <div className="flex flex-col flex-1 relative" ref={partyDropdownRef}>
                ...
                {/* UPDATED PARTY DROPDOWN DISABLE */}
          ...
          <div className="p-4 bg-white shrink-0">
            ...
            <div className="grid grid-cols-3 gap-6 pt-3">
              <div className="flex flex-col gap-4 border-r border-gray-200 pr-6">
                ...
                {/* PASS isViewMode down to disable the dropdown */}
                <AccountDropdown 
                  label="Account"
              ...
              <div className="flex flex-col gap-3 border-r border-gray-200 pr-6">
                <div className="flex flex-col flex-1 relative mt-2" ref={labourDropdownRef}>
                  ...
                  {/* UPDATED LABOUR DROPDOWN DISABLE */}
        ...
        <div className="bg-[#f8fafc] px-3 py-2.5 flex justify-between items-center text-xs shrink-0 z-10">
          <button className="bg-[#1e73be] text-white px-5 py-1.5 rounded font-medium shadow-sm hover:bg-blue-700 transition-colors">Print</button>
          <div className="flex gap-2">
            {/* HIDE SAVE BUTTONS IN VIEW MODE */}
            {!isViewMode && <button onClick={() => handleSave(false)} className="bg-[#1e73be] text-white px-5 py-1.5 rounded font-medium shadow-sm hover:bg-blue-700 transition-colors">Save (F3)</button>}
            {!isViewMode && <button onClick={() => handleSave(true)} className="bg-[#1e73be] text-white px-5 py-1.5 rounded font-medium shadow-sm hover:bg-blue-700 transition-colors">Save & Close (F4)</button>}
            
            <button onClick={onClose} className="bg-gray-600 text-white px-5 py-1.5 rounded font-medium shadow-sm hover:bg-gray-700 flex items-center gap-1 transition-colors">
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white w-full max-w-5xl flex flex-col shadow-2xl border border-gray-400 text-xs rounded-sm">
        <div className="bg-[#1e73be] text-white px-3 py-1.5 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button className="hover:bg-blue-700 px-1.5 rounded">📄</button><button className="hover:bg-blue-700 px-1.5 rounded">🔍</button><button onClick={onClose} className="hover:bg-red-500 hover:text-white px-2 py-0.5 rounded bg-white text-black font-bold">✕</button>
          </div>
        </div>
        ...
        <div className="bg-[#e2e8f0] px-3 py-2 flex justify-between border-t items-center">
          ...
          <div className="flex gap-2">
            <button onClick={() => { if(accName) onSave({ name: accName, city: accCity, gstNo: accGstNo }); }} className="bg-[#1e73be] text-white px-6 py-1.5 rounded font-medium shadow-sm hover:bg-blue-700">Save (F3)</button>
            <button onClick={() => { if(accName) onSave({ name: accName, city: accCity, gstNo: accGstNo }); }} className="bg-[#1e73be] text-white px-6 py-1.5 rounded font-medium shadow-sm hover:bg-blue-700">Save & Close (F4)</button>
