...
export const generateInwardOutwardPdf = (record, transportData = null, mode = "print") => {
  ...
  // For Outward prints, show the delivery/outward date — not the original inward entry date
  ...
  // ── Entry No / Date / Delivery Date / LR No ──────────────────────────────────
  const cW = W / 4;
  sz(7); n();
  doc.text("LR No.",        M,          y);
  doc.text("Date",          M + cW,     y);
  doc.text("Delivery Date", M + 2 * cW, y);
  doc.text("Entry No.",     M + 3 * cW, y);
  y += 4;

  sz(10); b();
  doc.text(record.lrNo || "-",         M,          y);
  sz(8.5); n();
  doc.text(date,                       M + cW,     y);
  doc.text(deliveryDate,               M + 2 * cW, y);
  doc.text(entryNo,                    M + 3 * cW, y);
  y += 6;
  ...
  if (goodsDesc) {
    ...
    ly += gLines.slice(0, 2).length * 3.8 + 1;
  }
  if (totalArt > 0 || totalWt > 0) {
    ly += 1;
    sz(8); b();
    doc.text(`Articles: ${totalArt || "-"}`, M, ly);
    n(); doc.text(`Weight: ${totalWt > 0 ? totalWt + " Kg" : "-"}`, M + 44, ly);
    ly += 5;
  }

  // ── Jurisdiction (left) · Driver & Vehicle (right of jurisdiction) ──────────
  const jurisdictionCity = transportData?.jurisdictionCity || "";
  const infoColX = M + 52;  // x start for driver/vehicle column within left side
  const hasInfo  = jurisdictionCity || driver || vehicleNo;

  if (hasInfo) {
    ly += 1;
    // Row 1: Jurisdiction (left) + Driver (right)
    if (jurisdictionCity) {
      sz(6.5); n(); doc.text("Jurisdiction:", M, ly);
      sz(7.5); b(); doc.text(jurisdictionCity, M + 21, ly, { maxWidth: 28 });
    }
    if (driver) {
      sz(6.5); n(); doc.text("Driver:", infoColX, ly);
      sz(7.5); b(); doc.text(driver, infoColX + 13, ly, { maxWidth: midX - infoColX - 15 });
    }
    ly += 4.5;

    // Row 2: Vehicle No (right column only)
    if (vehicleNo) {
      sz(6.5); n(); doc.text("Vehicle No:", infoColX, ly);
      sz(7.5); b(); doc.text(vehicleNo, infoColX + 19, ly, { maxWidth: midX - infoColX - 20 });
      ly += 4.5;
    }
    n();
  }
  ...
  ry += cneeLines.slice(0, 2).length * 4 + 5;

  sz(8.5); n();
  doc.text("TOTAL FREIGHT", RX, ry);
  doc.text(String(totalFreight || 0), RE, ry, { align: "right" });
  ry += 5;

  if (hamali > 0) {
    doc.text("HAMALI", RX, ry);
    doc.text(String(hamali), RE, ry, { align: "right" });
    ry += 5;
  }

  if (serviceCharge > 0) {
    doc.text("SERVICE CHARGE", RX, ry);
    doc.text(String(serviceCharge), RE, ry, { align: "right" });
    ry += 5;
  }

  if (demurrageAmt > 0) {
    doc.text("DEMURRAGE", RX, ry);
    doc.text(String(demurrageAmt), RE, ry, { align: "right" });
    ry += 5;
  }

  if (discount > 0) {
    doc.text("DISCOUNT", RX, ry);
    doc.text(`-${discount}`, RE, ry, { align: "right" });
    ry += 5;
  }
  ...
  sz(10); b();
  // DeliveryFreight = TotalFreight - Discount (hamali/service are our income, not added to party total)
  ...
  if (deliveryBy)   { doc.text(`Delivered By: ${deliveryBy}`, RX, ry); ry += 4; }
