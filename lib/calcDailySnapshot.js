...
export async function calcSnapshot(transportSlug, dateStr) {
  ...
  // Use yesterday's cached closing balance as today's opening — avoids scanning
  // all historical data on every dashboard load. Only falls back to full
  // calcOpening() on the very first day or after a cache miss.
  const yesterday = shiftDate(dateStr, -1);
  const prevSnap = await DailySnapshot.findOne({ transportSlug, date: yesterday }).lean();
    ...
    await Promise.all([
      ...
      safeAgg(InwardOutward, [
        ...
        { $group: {
          _id: null,
          serviceChargeIncome: { $sum: { $convert: { input: "$deliveryData.serviceCharge", to: "double", onError: 0, onNull: 0 } } },
          demurrageIncome:     { $sum: { $convert: { input: "$deliveryData.demurrageAmt", to: "double", onError: 0, onNull: 0 } } },
          ioHamaliExpense:     { $sum: { $convert: { input: "$deliveryData.hamali",        to: "double", onError: 0, onNull: 0 } } },
        }},
      ]),
      prevSnap ? Promise.resolve(prevSnap.closingBalance) : calcOpening(transportSlug, dateStr),
    ]);
...
export async function bulkCalcRange(slugs, from, to, allExpenses = false) {
  ...
  if (runningBalance === null) {
    ...
    const ioBefore      = { $lt: new Date(from + "T00:00:00.000Z") };
    const [expH, salH, lrH, memoH, ioH] = await Promise.all([
      safeAgg(Expense, [
      ...
      safeAgg(InwardOutward, [
        { $match: { transportSlug: { $in: slugList }, createdAt: ioBefore } },
        { $group: {
          _id: null,
          serviceChargeIncome: { $sum: { $convert: { input: "$deliveryData.serviceCharge", to: "double", onError: 0, onNull: 0 } } },
          demurrageIncome:     { $sum: { $convert: { input: "$deliveryData.demurrageAmt", to: "double", onError: 0, onNull: 0 } } },
    ...
    const histIncome =
      (lrH[0]?.income               || 0) +
      (ioH[0]?.serviceChargeIncome  || 0) +
      (ioH[0]?.demurrageIncome      || 0);
    const histExpenses =
