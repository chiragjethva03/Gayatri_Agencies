...
// Works without cached snapshots; accurate even on first launch.
async function calcOpening(transportSlug, dateStr) {
  const before = { $lt: dateStr };

  const [delH, expH, salH, lrH, memoH, demH] = await Promise.all([
    safeAgg(Delivery, [
      { $match: { transportSlug, date: before } },
      { $group: { _id: null, gross: { $sum: "$delSubTotal" }, kasar: { $sum: { $ifNull: ["$kasar", 0] } } } },
    ]),
    safeAgg(Expense, [
      { $match: { transportSlug, date: before } },
    ...
    safeAgg(Delivery, [
      { $match: { transportSlug, date: before, demurrageStatus: "paid" } },
      { $group: { _id: null, total: { $sum: { $ifNull: ["$demurragePaidAmt", 0] } } } },
    ]),
  ]);

  const histIncome =
    ((delH[0]?.gross || 0) - (delH[0]?.kasar || 0)) +
    (lrH[0]?.income  || 0) +
    (demH[0]?.total  || 0);
...
export async function calcSnapshot(transportSlug, dateStr) {
  ...
  const [deliveryAgg, expenseAgg, salaryAgg, lrAgg, memoAgg, demurrageAgg, openingBalance] =
    await Promise.all([
      safeAgg(Delivery, [
        { $match: match },
        { $group: { _id: null, deliveryGross: { $sum: "$delSubTotal" }, kasarTotal: { $sum: { $ifNull: ["$kasar", 0] } } } },
      ]),
      safeAgg(Expense, [
        { $match: match },
        { $group: { _id: null, total: { $sum: "$amount" } } },
      ...
      safeAgg(Delivery, [
        { $match: { ...match, demurrageStatus: "paid" } },
        { $group: { _id: null, total: { $sum: { $ifNull: ["$demurragePaidAmt", 0] } } } },
      ]),
      calcOpening(transportSlug, dateStr),
    ]);

  const deliveryGross   = deliveryAgg[0]?.deliveryGross  || 0;
  const kasarTotal      = deliveryAgg[0]?.kasarTotal     || 0;
  const deliveryIncome  = deliveryGross - kasarTotal;
  const paidLrIncome    = lrAgg[0]?.paidLrIncome         || 0;
  const demurrageIncome = demurrageAgg[0]?.total          || 0;
  const totalIncome     = deliveryIncome + paidLrIncome + demurrageIncome;
...
export async function bulkCalcRange(slugs, from, to, allExpenses = false) {
  ...
  // When allExpenses=true (EOD "all transports" view), skip the transportSlug
  // filter so global expenses (added from /dashboard/expense with no slug) are included.
  const expenseMatch = allExpenses
    ? { date: dateFilter }
    : { transportSlug: { $in: slugList }, date: dateFilter };

  const [deliveryAgg, expenseAgg, salaryAgg, lrAgg, memoAgg, demurrageAgg] = await Promise.all([
    safeAgg(Delivery, [
      { $match: { transportSlug: { $in: slugList }, date: dateFilter } },
      { $group: { _id: "$date", deliveryGross: { $sum: "$delSubTotal" }, kasarTotal: { $sum: { $ifNull: ["$kasar", 0] } } } },
    ]),
    safeAgg(Expense, [
      { $match: expenseMatch },
    ...
    safeAgg(Delivery, [
      { $match: { transportSlug: { $in: slugList }, date: dateFilter, demurrageStatus: "paid" } },
      { $group: { _id: "$date", total: { $sum: { $ifNull: ["$demurragePaidAmt", 0] } } } },
    ]),
  ]);

  const byDate       = arr => Object.fromEntries(arr.map(r => [r._id, r]));
  const deliveryMap  = byDate(deliveryAgg);
  const expenseMap   = byDate(expenseAgg);
  const salaryMap    = byDate(salaryAgg);
  const lrMap        = byDate(lrAgg);
  const memoMap      = byDate(memoAgg);
  const demMap       = byDate(demurrageAgg);

  // Opening balance for the range: cumulative P&L before 'from' date
  // Try cached snapshot first (fast), else compute live (accurate)
  ...
  if (runningBalance === null) {
    // No cached snapshot — compute cumulative opening from raw data
    ...
    const [delH, expH, salH, lrH, memoH, demH] = await Promise.all([
      ...
      safeAgg(Delivery, [
        { $match: { transportSlug: { $in: slugList }, date: beforeFilter, demurrageStatus: "paid" } },
        { $group: { _id: null, total: { $sum: { $ifNull: ["$demurragePaidAmt", 0] } } } },
      ]),
    ]);
  ...
  for (const d of dateRange(from, to)) {
    ...
    const deliveryGross   = del.deliveryGross   || 0;
    const kasarTotal      = del.kasarTotal      || 0;
    const deliveryIncome  = deliveryGross - kasarTotal;
    const paidLrIncome    = lr.paidLrIncome     || 0;
    const demurrageIncome = dem.total           || 0;
    const totalIncome     = deliveryIncome + paidLrIncome + demurrageIncome;
