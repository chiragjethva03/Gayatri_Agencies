...
export const generateMemoPdf = (memoData, mode = "download") => {
  ...
  const toPayLrTotalN  = Number(memoData.toPayLrTotal ?? lrListEarly.filter(l => (l.freightBy||"").toLowerCase() !== "paid" ).reduce((s,l) => s+(Number(l.freight)||0), 0));
  ...
  const tbbN           = memoData.tbb            != null ? Number(memoData.tbb)            : Math.max(0, toPayLrTotalN - toPayN);
  ...
  doc.setFont("helvetica", "bold"); doc.setTextColor(0,0,0); doc.text(truckBalance, 180, y + 12);
  ...
  if (lrList.length > 0) {
    lrList.forEach(lr => {
      ...
      tableRows.push([
        ...
        lr.consignee || "-",
        art.toString(),
        [lr.packaging, lr.description || lr.goodsContain].filter(v => v && v !== "-").join(" / ") || "-",
        isPaid ? `(PAID) ${frt.toFixed(2)}` : frt.toFixed(2),
        lr.remarks || ""
      ]);
    });
  } else {
  ...
  autoTable(doc, {
    ...
    theme: 'grid',
    headStyles: { fillColor: [240, 242, 245], textColor: [30, 30, 30], fontStyle: 'bold' },
    footStyles: { fillColor: [255, 255, 255], textColor: [0, 0, 0],   fontStyle: 'bold' },
    columnStyles: {
      3: { halign: 'center' },
      5: { halign: 'right'  },
    },
    styles: { fontSize: 9, cellPadding: 3, lineColor: [200, 200, 200], textColor: [50, 50, 50] },
