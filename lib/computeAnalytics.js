...
export async function computeAnalytics(transport = "all", period = "month") {
  ...
  // Live demurrage: recalculate per-entry using stored rate & free-days so the
  // total reflects today's actual accrued charge, not the frozen DB value.
  // Period-filtered by inward date; entries without a stored rate fall back to saved amount.
  const todayMs = new Date().setHours(0, 0, 0, 0);

  const ioPeriod = await InwardOutward.find({
    type: "Inward",
    ...(transport !== "all" ? { transportSlug: transport } : {}),
    date: { $gte: fromCur, $lte: toCur },
  }).lean();

  totals.demurrageIncome = ioPeriod.reduce((s, io) => {
    const dlv      = io.deliveryData || {};
    const rate     = Number(dlv.demurrageRate) || 0;
    const freeDays = Number(dlv.demurrageDays) >= 0 ? Number(dlv.demurrageDays) : 7;

    // Old entries without a stored rate — use the frozen saved amount
    if (!rate) return s + (Number(dlv.demurrageAmt) || 0);

    const startMs    = new Date(io.date).setHours(0, 0, 0, 0);
    const totalDays  = Math.max(0, Math.floor((todayMs - startMs) / 86400000) - 1);
    const chargeDays = Math.max(0, totalDays - freeDays);
    return s + (chargeDays * rate);
  }, 0);
